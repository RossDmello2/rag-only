...
const SESSION_KEY = 'ross-static-rag-openai-key';
...
function bindUi() {
  ...
  ui.cloudModelInput = document.getElementById('cloudModelInput');
  ui.useCloudInput = document.getElementById('useCloudInput');
  ui.openAiKeyInput = document.getElementById('openAiKeyInput');
...
function wireEvents() {
  ...
  ui.useCloudInput.addEventListener('change', () => {
    applyModeBadge(ui.useCloudInput.checked);
  });
...
function openPanelWithWelcome() {
  ...
  appendMessage('assistant', [
    ...
    'Flow: ingest locally -> embed with Ollama -> store in Qdrant -> retrieve -> answer with local or cloud chat model.',
...
function loadSettings() {
  try {
    ...
    if (sessionKey) state.settings.openAiApiKey = sessionKey;
  } catch {
...
function populateSettingsModal() {
  ...
  ui.cloudModelInput.value = state.settings.cloudModel;
  ui.useCloudInput.checked = state.settings.useCloud;
  ui.openAiKeyInput.value = state.settings.openAiApiKey || '';
...
function applyModeBadge(useCloud = state.settings.useCloud) {
  ui.modeBadge.textContent = useCloud ? 'Cloud' : 'Local';
}

function saveSettingsFromModal() {
  const newSettings = {
    ollamaBaseUrl: normalizeUrl(ui.ollamaUrlInput.value),
    qdrantBaseUrl: normalizeUrl(ui.qdrantUrlInput.value),
    ...
    cloudModel: ui.cloudModelInput.value.trim(),
    useCloud: ui.useCloudInput.checked,
    openAiApiKey: ui.openAiKeyInput.value.trim(),
  ...
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...newSettings, openAiApiKey: '' }));

  if (newSettings.openAiApiKey) {
    sessionStorage.setItem(SESSION_KEY, newSettings.openAiApiKey);
  } else {
...
async function ingestFile(file) {
  ...
  const existing = await fileAlreadyIndexed(fileHash);
  if (existing) {
    return { deduped: true, fileHash, fileName: file.name };
  }
...
function splitText(text, chunkSize, chunkOverlap) {
  ...
  for (const segment of segments) {
    ...
    for (const word of words) {
      ...
      if (candidate.length <= chunkSize) {
        ...
        const overlap = buffer.slice(-chunkOverlap).trim();
  ...
  chunks.forEach((chunk, index) => {
    ...
    const previousTail = withOverlap[index - 1].slice(-chunkOverlap).trim();
    const merged = previousTail && !chunk.startsWith(previousTail)
      ? `${previousTail} ${chunk}`.slice(0, chunkSize + chunkOverlap)
      : chunk;
...
async function fileAlreadyIndexed(fileHash) {
...
async function openAiChat(messages) {
  if (!state.settings.openAiApiKey) {
    throw new Error('OpenAI API key is missing. Open settings and paste it for this tab session.');
  }

  if (!state.settings.cloudModel) {
    throw new Error('Cloud chat model is missing. Enter your OpenAI model ID in settings.');
  }
  ...
  const response = await fetchJson('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: state.settings.cloudModel,
      messages,
      temperature: 0.15,
      max_tokens: 1200,
    })
...
function formatError(error) {
  ...
  if (/401/.test(message)) return 'Authentication failed. Check your OpenAI API key.';
